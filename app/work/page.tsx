...
import { findSymbolDescription } from "@/symbol-descriptions";
...
function DrawingViewer({ task, isRework }: { task: WorkTaskDetail; isRework: boolean }) {
  ...
  const handleImageTap = async (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    try {
      ...
      setSymbolInfo(data);
      if (data.name_en) {
        const matched = findSymbolDescription(data.name_en);
        setMatchedImageUrl(matched?.imageUrl ?? null);
      }
    } catch {
  ...
  return (
    <>
      <div
        className={`relative bg-[#1E2A3A] w-full ${hasImage ? "cursor-crosshair" : ""}`}
        style={{ height: "300px" }}
        onClick={hasImage ? handleImageTap : undefined}
      >
        {hasImage ? (
          <>
            <img
            ...
            {tapPoint && (
              ...
              >
                {analyzing ? (
                  ...
                  <div className="w-10 h-10 rounded-full border-2 border-blue-400 bg-blue-500/30 flex items-center justify-center shadow-lg">
            ...
            {!tapPoint && (
              <div className="absolute inset-0 flex items-end justify-center pb-10 pointer-events-none">
                <div className="bg-black/60 backdrop-blur-sm rounded-full px-4 py-2 flex items-center gap-2">
                  ...
                  <span className="text-white text-xs font-semibold">심볼을 탭하여 AI 분석</span>
                </div>
              </div>
            )}
      ...
      {/* 심볼 정보 팝업 */}
      {symbolInfo && (
        <div className="fixed bottom-[76px] left-0 right-0 max-w-sm mx-auto z-40 bg-white rounded-t-3xl shadow-2xl max-h-[50vh] overflow-y-auto">
          <div className="px-5 pt-5 pb-6">
            <div className="flex items-start justify-between mb-3">
              ...
              <button
                type="button"
                onClick={() => { setSymbolInfo(null); setTapPoint(null); setMatchedImageUrl(null); }}
            ...
            {!symbolInfo.error && (
              <>
                ...
                {matchedImageUrl && (
                  <div className="mb-4 rounded-2xl overflow-hidden bg-gray-50 flex items-center justify-center" style={{ height: "160px" }}>
                    <img src={matchedImageUrl} alt={symbolInfo.name_en} className="h-full w-full object-contain" />
                  </div>
                )}
